...
- **Zero-Friction Cloud & Local Operations**: Support 100% free multi-service cloud deployment across Render and Vercel (`render.yaml` Blueprint), as well as zero-setup local development with in-memory fallbacks.
...
- **Action Recording**: Listens for `click`, `input`, `change`, `form submit`, `blur`, and `history.pushState` navigation events.
- **Input De-Duplication**: De-duplicates rapid input keystrokes into clean single-step text values.
...
- **Endpoint Auto-Normalization**: Normalizes any input host to `/api/recordings` (`normalizeRecordingsUrl()`) and provides diagnostic error reporting.

### 3.2 Backend API & WebSocket Engine (`backend/`)
- **REST Endpoints**:
  - `POST /api/recordings` & `POST /recordings`: Save recorded step sequences as workflow definitions.
  - `POST /api/workflows/from-template`: Create pre-populated workflows from starter JSON fixtures (`report-download`, `form-fill`, `page-watch`), backed by in-memory fallback steps.
  - `GET /api/workflows`: List stored workflows with step counts, schedules, latest run IDs, and last run status (`success` | `failed` | `awaiting_approval` | `never_run`).
  - `GET /api/workflows/:id`: Retrieve full workflow definition and step array.
  - `POST /api/workflows/:id/run`: Enqueue workflow execution.
  - `GET /api/runs/pending` & `POST /api/runs/:id/claim`: Worker polling and atomic job claiming bridge.
  - `POST /api/runs/:id/upload-result`: Accept binary report uploads (`application/octet-stream`) from workers.
  - `GET /api/runs/:id/download` & `GET /api/runs/:id/preview`: Serve report files directly to web browsers.
...
- **Isolated Browser Context**: Spawns isolated Chromium instances (`headless: false` locally, `headless: true` in cloud) per run.
- **Embedded Health Server**: Embedded HTTP server listening on `$PORT` to support Render's 100% free web service tier.
- **Backend URL Resolver**: `resolveBackendUrl()` validates and auto-prefixes missing `https://` schemes.
...
- **File Upload Sync**: Streams downloaded report files to backend via `POST /api/runs/:id/upload-result`.
...
- **Workflow Dashboard**: Card grid listing workflows, step counts, quick run triggers, and direct click-throughs to approval run pages.
- **Dynamic Backend Configurator**: `Backend Connected ⚙️` pill in Navbar enabling 1-click backend URL updates persisted in `localStorage`.
- **WebSocket Protocol Derivation**: `getWsBase()` automatically resolves `wss://` for HTTPS Vercel pages.
