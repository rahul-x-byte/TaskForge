...
export async function testDatabaseConnection(): Promise<{ connected: boolean; error?: string; mode: string }> {
  if (!pgPool) {
    return {
      connected: false,
...
export async function withTransaction<T>(
  callback: (client: { query: (text: string, params?: any[]) => Promise<{ rows: any[]; rowCount?: number | null }> }) => Promise<T>
): Promise<T> {
