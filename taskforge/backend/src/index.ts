  ...
  try {
    ...
    if (!signUpError && signUpData?.user) {
      ...
      if (adminError || !adminData?.user) {
        ...
        userId = uuidv4();
        const memUser = { id: userId, name, email, role: 'user' as const, created_at: new Date().toISOString() };
    ...
    await pool.query(
      `INSERT INTO profiles (id, name, email, role, created_at, updated_at)
       ...
       ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, email = EXCLUDED.email, role = 'user'`,
      [userId, name, email]
    );
    ...
    const userProfile = { id: userId, name, email, role: 'user' as const, created_at: new Date().toISOString() };
...
app.post('/api/workflows', { preHandler: [requireAuth] }, async (request, reply) => {
  ...
  await pool.query(
    ...
    [workflowId, name, user.id, versionId]
  );
...
app.post('/api/workflows/from-template', { preHandler: [requireAuth] }, async (request, reply) => {
  ...
  await pool.query(
    'INSERT INTO workflows (id, name, user_id, current_version_id) VALUES ($1, $2, $3, $4)',
    [workflowId, templateData.name, user.id, versionId]
  );

  await pool.query(
    'INSERT INTO workflow_versions (id, workflow_id, steps) VALUES ($1, $2, $3)',
    [versionId, workflowId, JSON.stringify(templateData.steps || [])]
  );
  ...
  if (Array.isArray(body.steps)) {
    ...
    if (wf.current_version_id) {
      await pool.query(
        ...
        [JSON.stringify(body.steps), wf.current_version_id]
      );
    } else {
      const newVerId = uuidv4();
      await pool.query(
        'INSERT INTO workflow_versions (id, workflow_id, steps) VALUES ($1, $2, $3)',
        [newVerId, id, JSON.stringify(body.steps)]
      );
...
app.put('/api/workflows/:id/steps', { preHandler: [requireAuth] }, async (request, reply) => {
  ...
  await pool.query(
    'UPDATE workflow_versions SET steps = $1 WHERE id = $2',
    [JSON.stringify(steps), wf.current_version_id]
  );
...
app.delete('/api/workflows/:id', { preHandler: [requireAuth] }, async (request, reply) => {
  ...
  const deletedCount = res.rows ? res.rows.length : (res as any).rowCount || 0;
  if (deletedCount === 0) {
    return reply.status(404).send({ error: 'Not Found', message: 'Workflow not found or access denied.' });
  }
...
app.post('/api/workflows/:id/schedule', { preHandler: [requireAuth] }, async (request, reply) => {
  ...
  await pool.query(
    'INSERT INTO schedules (id, user_id, workflow_id, frequency, time, enabled) VALUES ($1, $2, $3, $4, $5, true)',
    [uuidv4(), user.id, id, frequency || 'daily', time || '09:00']
  );
...
app.put('/api/admin/users/:id/role', { preHandler: [requireAdmin] }, async (request, reply) => {
  const { id } = request.params as { id: string };
  const body = request.body as any;
  const targetRole: 'admin' | 'user' = body?.role === 'admin' ? 'admin' : 'user';
  ...
  if (memUser) memUser.role = targetRole;
...
app.post('/api/runs/:id/claim', { preHandler: [verifyWorkerSecret] }, async (request, reply) => {
  ...
  if (res.rows.length === 0) return reply.status(409).send({ error: 'Run already claimed or not pending' });
  return reply.send(res.rows[0]);
});

app.post('/api/runs/:id/upload-result', { preHandler: [verifyWorkerSecret] }, async (request, reply) => {
  const { id } = request.params as { id: string };
  const filename = (request.headers['x-filename'] as string) || `${id}.csv`;
  const filePath = path.join(uploadsDir, filename);
...
app.patch('/api/runs/:id/status', { preHandler: [verifyWorkerSecret] }, async (request, reply) => {
  ...
  await pool.query(
    'UPDATE runs SET status = $1, finished_at = CASE WHEN $1 IN (\'completed\', \'failed\', \'cancelled\') THEN NOW() ELSE finished_at END WHERE id = $2',
    [status, id]
  );
...
app.get('/ws/runs/:id', { websocket: true }, async (connection: any, req) => {
