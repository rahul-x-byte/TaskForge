...
app.post('/api/recordings', async (request, reply) => {
    ...
    try {
        await pool.query('INSERT INTO profiles (id, name, email, role) VALUES ($1, $2, $3, $4) ON CONFLICT (id) DO NOTHING', [user.id, user.name || 'User', user.email || '', user.role || 'user']);
    }
    catch (profErr) {
        try {
            await pool.query('INSERT INTO users (id, name, email, password_hash, role) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (id) DO NOTHING', [user.id, user.name || 'User', user.email || '', 'managed_by_supabase', user.role || 'user']);
        }
        catch (uErr) { }
    }
