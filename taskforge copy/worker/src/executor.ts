...
function resolveSelector(page: Page, selectors?: SelectorBundle): Locator {
  if (!selectors) {
  ...
  if (selectors.role && selectors.name) {
    try {
      return page.getByRole(selectors.role as any, { name: selectors.name });
    } catch (e) {}
  }
...
async function waitForApprovalGate(runId: string, stepIndex: number, stepDetail: any): Promise<boolean> {
