...
import { fetchWorkflowById, saveWorkflowSchedule, triggerWorkflowRun, WorkflowItem } from '../api';
    ...
    fetchWorkflowById(workflowId)
      .then((data) => {
        ...
        const flowNodes: Node[] = steps.map((step: any, idx: number) => {
          ...
          return {
            id: `node-${idx}`,
            position: { x: 180, y: idx * 120 + 40 },
            data: {
              label: (
                <div style={{ padding: '8px', minWidth: '220px', background: isSensitive ? '#450a0a' : '#1e293b', border: isSensitive ? '1px solid #f43f5e' : '1px solid #475569', borderRadius: '8px', color: '#f8fafc' }}>
